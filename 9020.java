...
public class Main{
		...
		public static void main(String[] args) throws NumberFormatException, IOException{
			...
			int result[] = new int[10001];//소수 값을 담을 배열 
			...
			while(count<testcase){
				...
				int z=0;
					...
					int min = 1000000;//min값 절대 나올 수 없는 수로 초기화
						...
						if(result[i]!=0 && result[i+1]!=0) {
							if(result[i+1]-result[i]<min) {
								if(result[i+1]-result[i]<0) {
									break Loop1;
								}//min이 음수로 나오면 반복문 빠져나옴 (뒤는 검사해도 의미없으므로)
								
								min=result[i+1]-result[i];//min갱신
								index_number=i;//몇번째 인덱스에 최솟값 저장?(작은 수의 index저장)
